...
async function handleScreenshot(
  ...
  try {
    ...
    if (!savePath) {
      ...
      const screenshotDir = join(getAppDir(), 'tmp', 'screenshots');
      mkdirSync(screenshotDir, { recursive: true });
      savePath = join(screenshotDir, filename);
    }
...
async function handleStateLoad(
  ...
  if (!(await Bun.file(command.path).exists())) {
...
async function handleStateClear(command: StateClearCommand): Promise<Response> {
  ...
  if (command.all) {
    for (const file of files) {
      await Bun.file(join(sessionsDir, file)).delete();
      deleted.push(file);
    }
...
async function handleStateShow(command: StateShowCommand): Promise<Response> {
  ...
  const filepath = join(sessionsDir, command.filename);
  ...
  try {
    const { data: state, wasEncrypted } = await readStateFile(filepath);
    const stats = await stateFile.stat();
...
async function handleStateClean(command: StateCleanCommand): Promise<Response> {
  const deleted = await cleanupExpiredStates(command.days);
...
async function handleStateRename(command: StateRenameCommand): Promise<Response> {
  ...
  const oldPath = join(sessionsDir, `${command.oldName}.json`);
  const newPath = join(sessionsDir, `${command.newName}.json`);

  let oldFile = Bun.file(oldPath);
