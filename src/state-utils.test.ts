...
describe('state-utils', () => {
  beforeEach(() => {
    tempHome = mkdtempSync(join(tmpdir(), 'agent-browser-test-'));
  });
  ...
  describe('getAutoStateFilePath', () => {
    ...
    it('should always produce path within sessions directory', () => {
      ...
      const resolvedPath = resolve(result!);
  ...
  describe('cleanupExpiredStates', () => {
    it('should return empty array for 0 days', async () => {
      const result = await cleanupExpiredStates(0);
      expect(result).toEqual([]);
    });

    it('should return empty array for negative days', async () => {
      const result = await cleanupExpiredStates(-5);
      expect(result).toEqual([]);
    });
  });
});
