...
describe('BrowserManager', () => {
  ...
  describe('cursor-ref selector uniqueness', () => {
    it('should produce unique selectors for repeated DOM structures', async () => {
      ...
      // Each ref's selector must be unique
  ...
  describe('scoped headers', () => {
    ...
    it('should clear scoped headers for specific origin', async () => {
      ...
      await expect(browser.clearScopedHeaders('https://example.com')).resolves.toBeUndefined();
    });
    ...
    it('should clear all scoped headers', async () => {
      ...
      await expect(browser.clearScopedHeaders()).resolves.toBeUndefined();
    });
    ...
    it('should handle clearing non-existent origin gracefully', async () => {
      ...
      await expect(browser.clearScopedHeaders('https://never-set.com')).resolves.toBeUndefined();
    });
  });
    ...
    it('should filter out pages with empty URLs during CDP connection', async () => {
      const mockBrowser = {
        contexts: () => [
          {
            pages: () => [
              { url: () => 'http://example.com', on: mock() },
              { url: () => '', on: mock() }, // This page should be filtered out
              { url: () => 'http://anothersite.com', on: mock() },
            ],
            on: mock(),
      ...
      const spy = spyOn(chromium, 'connectOverCDP').mockResolvedValue(mockBrowser as any);
  ...
  describe('screencast', () => {
    ...
    it('should handle stop when not screencasting', async () => {
      // Should not throw
      await expect(browser.stopScreencast()).resolves.toBeUndefined();
    });
  });
