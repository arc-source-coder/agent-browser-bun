...
export function getSessionsDir(): string {
  return join(homedir(), '.agent-browser', 'sessions');
}
...
export function getAutoStateFilePath(sessionName: string, sessionId: string): string | null {
  ...
  return join(sessionsDir, `${sessionName}-${sessionId}.json`);
}
...
export async function readStateFile(
  filepath: string
): Promise<{ data: object; wasEncrypted: boolean }> {
 ...
 */
export async function cleanupExpiredStates(days: number): Promise<string[]> {
  if (days <= 0) return [];
