...
async function saveStateToFile(
  ...
  if (key) {
    ...
    await Bun.write(filepath, JSON.stringify(encrypted, null, 2));
...
async function runCleanupExpiredStates(): Promise<void> {
  ...
  try {
    const deleted = await cleanupExpiredStates(expireDays);
 ...
 */
async function getSessionAutoStatePath(): Promise<string | undefined> {
  ...
  try {
    ...
    if (!autoStatePath) return undefined;
    return (await Bun.file(autoStatePath).exists()) ? autoStatePath : undefined;
  } catch {
    return undefined;
  }
}
        ...
        async data(socket, data) {
          ...
          while (buffer.includes('\n')) {
            ...
            try {
              ...
              // Handle device_list specially - works without a session, always uses IOSManager
              ...
              if (
                ...
                if (isIOS && manager instanceof IOSManager) {
                  ...
                  await manager.launch({
                    ...
                    autoStateFilePath: await getSessionAutoStatePath(),
                  });
                }
              }
              ...
              if (
                ...
                const autoStatePath = await getSessionAutoStatePath();
                if (autoStatePath) {
                  parseResult.command.autoStateFilePath = autoStatePath;
                }
              }

              // Handle close command specially - triggers full shutdown
