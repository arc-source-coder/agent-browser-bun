...
describe('getSocketDir', () => {
  ...
  describe('XDG_RUNTIME_DIR', () => {
    it('should use when AGENT_BROWSER_SOCKET_DIR is not set', () => {
      process.env.XDG_RUNTIME_DIR = 'run/user/1000';
      // This is needed to ensure the test doesn't fail with windows-style paths (\)
      let expected_dir = join('run', 'user', '1000', 'agent-browser');
      expect(getSocketDir()).toBe(expected_dir);
    });
  ...
  describe('fallback', () => {
    it('should use home directory when env vars are not set', () => {
      const result = getSocketDir();
      const expected = join(homedir(), '.agent-browser');
